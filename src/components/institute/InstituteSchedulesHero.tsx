...
type Props = Readonly<{
  /** Batch schedule rounds with at least one pending interview */
  roundCount: number;
  /** Sum of pending candidate schedules across those rounds */
  pendingInterviewCount?: number;
  loading?: boolean;
}>;

export function InstituteSchedulesHero({
  roundCount,
      ...
      <div className="relative z-10 flex flex-col gap-3 px-5 py-5 sm:flex-row sm:items-center sm:justify-between sm:px-6 sm:py-6">
        <div className="min-w-0 space-y-1">
          ...
          <p className="text-sm text-white/80">
            Interviews are scheduled per batch — each round applies to every member in that
            cohort. Candidates start from 24 hours before the slot (saved resume required).
          </p>
        </div>
        ...
        {!loading ? (
          ...
          >
            ...
            <span className="text-sm font-medium text-white/90">
              ...
              batch round{roundCount === 1 ? "" : "s"}
              {typeof pendingInterviewCount === "number" && pendingInterviewCount > 0 ? (
                <span className="text-white/75">
                  {" "}
                  · {pendingInterviewCount.toLocaleString()} pending
                </span>
              ) : null}
            </span>
          </div>
