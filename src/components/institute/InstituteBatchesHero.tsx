...
export function InstituteBatchesHero({
  batchCount,
  totalMembers,
  loading,
}: Props) {
        ...
        <div className="min-w-0 space-y-1">
          ...
          <p className="text-sm text-white/80">
            {typeof totalMembers === "number" && totalMembers > 0
              ? `Group ${totalMembers.toLocaleString()} member${totalMembers === 1 ? "" : "s"} into cohorts and schedule as a group.`
              : "Create cohorts, add members, and run batch interview schedules."}
          </p>
        </div>
