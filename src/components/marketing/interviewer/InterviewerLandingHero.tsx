...
type InterviewerLandingHeroProps = {
  onBecomeInterviewer: () => void;
};

export function InterviewerLandingHero({
  onBecomeInterviewer,
}: Readonly<InterviewerLandingHeroProps>) {
  return (
    <section
      className={cn(
    ...
    >
        ...
        {[...Array(8)].map((_, i) => (
          ...
          >
            {i % 3 === 0 ? (
              ...
              <IndianRupee className="h-8 w-8 text-primary sm:h-11 sm:w-11" />
            )}
          </div>
        ))}
      </div>
      ...
      <div className="container relative z-10 mx-auto max-w-7xl">
        <div className="mb-8 flex justify-center sm:mb-10 lg:hidden">
          <Image
            src="/marketing/image-person-2.png"
            alt="Professional engineer conducting a mock interview on video"
            width={1400}
            height={1068}
            unoptimized
        ...
        <div className="grid items-center gap-8 sm:gap-12 lg:grid-cols-2 lg:gap-16">
          <div className="space-y-4 text-center sm:space-y-5 md:space-y-6 lg:max-w-xl lg:pb-24 lg:text-left">
            <div className="inline-flex items-center rounded-full bg-muted px-3 py-1 text-sm font-medium text-primary">
              <span>Become a Peer Interviewer</span>
            </div>
            ...
            <h1 className="mb-4 text-[1.22rem] font-bold leading-tight tracking-tight text-slate-900 sm:mb-6 sm:text-[1.46rem] md:text-[1.625rem] lg:text-[2.15rem] xl:text-[2.28rem]">
              <span className="block">Earn by mentoring candidates with</span>
              <span className="block">
                live <span className="text-primary">mock interviews</span>
              </span>
            </h1>

            <p className="mx-auto max-w-xl px-2 text-base leading-relaxed text-gray-600 sm:px-0 sm:text-lg lg:text-xl">
              Turn your interview experience into paid sessions. Set your own
              schedule and pricing, run DSA, system design, and HR rounds, and
              help candidates practice before their real hiring loops.
            </p>
            ...
            <div className="flex flex-col items-center justify-center gap-3 px-2 pt-2 sm:flex-row sm:gap-4 sm:px-0 lg:justify-start">
              ...
              >
                <Link
                  href={`/sign-in?redirect_url=${encodeURIComponent("/dashboard/peer-interviews/interviewer")}`}
                >
