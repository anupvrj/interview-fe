...
export default function EditAgentPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    useEffect(() => {
        const fetchAgent = async () => {
            try {
                setInitialLoad(true);
                const data = await researchApi.getAgent(resolvedParams.id);
                setFormData({
                    name: data.name || "",
                    version: data.version || "v1",
                    description: data.description || "",
                    environment: data.environment || "development",
                    spec: data.spec ? JSON.stringify(data.spec, null, 2) : JSON.stringify(DEFAULT_SPEC, null, 2)
                });
            } catch (err) {
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            await researchApi.updateAgent(resolvedParams.id, {
                ...formData,
                spec: parsedSpec
            });
