...
import { usePricingTrialOffer } from "@/hooks/usePricingTrialOffer";
import {
  appMarketingSection,
...
export default function PricingPage() {
  const { showBanner, state } = usePricingTrialOffer();
  ...
  return (
    <div className="min-h-screen scroll-smooth bg-background selection:bg-info-muted">
      ...
      <div className="pt-16 sm:pt-[4.25rem]">
        {showBanner ? <TrialPricingBanner visible /> : null}

        <section
          className={cn(
            "bg-background px-4 pb-12 sm:px-6 sm:pb-16 lg:pb-20",
            showBanner ? "pt-6 sm:pt-8" : "pt-8 sm:pt-10 lg:pt-12",
            state === "checking" && "transition-[padding] duration-200",
          )}
        >
        ...
        <section
          ...
          className={cn(
            appMarketingSection,
            "container mx-auto max-w-6xl scroll-mt-28 px-4 pb-16",
          )}
        >
