...
export class SarvamVoiceAgent extends BaseVoiceAgent {
  constructor(callbacks: VoiceAgentCallbacks) {
    super("sarvam", 24000, callbacks);
  }

  handleMessage(message: VoiceAgentMessage): void {
    switch (message.type) {
      case "connected":
        this.callbacks.onConnected?.("sarvam");
        break;
      case "preparing":
