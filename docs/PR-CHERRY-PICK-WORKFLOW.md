...
Use this when a teammate’s branch cannot merge cleanly (e.g. Vercel checks), or you need a fresh PR with the same commits rebased on `main`. A **reset** section is included for undoing a bad cherry-pick or amend.

**Monorepo:** The same guide (generic repo paths) is at **[GIT-WORKFLOW-GUIDELINES.md](../../GIT-WORKFLOW-GUIDELINES.md)** in the repository root.
...
## Step 6 — Reset when something went wrong

| Situation | Command |
|-----------|---------|
| Undo cherry-pick in progress | `git cherry-pick --abort` |
| Undo last commit, **keep** changes staged | `git reset --soft HEAD~1` |
| Undo last commit, **keep** changes unstaged | `git reset --mixed HEAD~1` (default) |
| **Discard** last commit and changes | `git reset --hard HEAD~1` — **destructive** |

**Already pushed?** Prefer `git revert` on shared branches, or coordinate `git push --force-with-lease`.
